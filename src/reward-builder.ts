...
export const buildRewards = async (
  ...
  const newRewards = editRewards(contractInfos, stipend)
  const finalRewards: Reward[] = []
  // hack for october, get best deal for each reward
  for (let i = 0; i < rewards.length; i++) {
    const finalReward = rewards[i].amount.gt(newRewards[i].amount)
      ? rewards[i]
      : newRewards[i]
